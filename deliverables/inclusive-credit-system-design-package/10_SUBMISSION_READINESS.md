...
|---|---|---|---|---|
| Detailed architecture and system design | Sections 1, 8, and 11 | Architecture documentation, twelve rendered figures, Mermaid sources, module map, runtime call paths | System status, lineage, governance views | Reference implementation and target-state controls are distinguished |
| Technical specifications | Sections 2, 13, and 14 | API schemas, feature contract, settings, security middleware, event and storage schemas | Contract-derived form, active model and threshold cards | Production SLOs and controls are acceptance targets unless measured |
| Component documentation | Sections 3, 11, and 12 | `src/ice/`, `services/api/`, `src/flg/`, `src/mie_credit_platform/`, `frontend/src/` | Scoring, explanation, fairness, portfolio, outcome, and governance views | Multiple Python namespaces remain a consolidation risk |
| Complete feature description | Section 12 | Nine-field principal contract, metadata, transforms, reason logic, API and UI capability map | Full feature grid and contract-driven form | Raw-source aggregation, missingness, and proxy validation remain production work |
| Model development and testing | Sections 4 and 15 and Appendices A-C | Training scripts, model abstraction, registry, automated tests, training report | Score, explanation, portfolio, reason-code behavior | Synthetic data and baseline performance cannot establish production fitness |
| Fairness and bias testing | Sections 5 and 15 | Fairness metric implementations, tests, thresholds, monitoring and acceptance design | Group counts, selection rates, parity and opportunity differences | Demonstration batch is synthetic; legal and statistical review remain required |
| Code and data contracts | Sections 11-15 and Appendices B-D | Current code excerpts, JSON, YAML, SQL, TypeScript, commands, pseudocode, and editable graphs | API-shaped demonstration payloads | Target-state blocks are labeled and are not implementation evidence |
